...
Dim X As Variant
    ...
    End If
End Function
'Получает путь и возвращает папку выше на указаную глубину
'путь получаем  по формуле Path=ПСТР(ЯЧЕЙКА("filename";A1);1;ПОИСК("[";ЯЧЕЙКА("filename";A1))-1)
'взято здесь:http://www.excel-vba.ru/chto-umeet-excel/otnositelnyj-put-k-dannym-powerquery/
'Пример D:\ISN AutoIt Studio\Projects\LIAS2017\Техкарта 2018\

'Получаем глубину Delimeter_Count(Path;Delimiter)-1
'Задаем вверх  число но не больше по  глубины -1

'Delimeter_Count(Path;"\")-1
Public Function SubstringPath(Path As Range, Delimiter, Cмещение) As String
Dim X As String
X = Delimiter
Глубина = Delimeter_Count(Path, X)
If Mid(Cмещение, 1, 1) = Delimiter Then
z = Path & Mid(Cмещение, 2, Len(Cмещение) - 1)
Else
For i = 1 To Глубина - Cмещение
If i = 1 Then
z = z & Substring(Path, Delimiter, i)
Else
z = z & "\" & Substring(Path, Delimiter, i)
End If
